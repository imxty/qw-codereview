...
【唯一检查项】
1. 语法错误：括号不匹配、缺少逗号/分号、关键字拼写错误、字符串未闭合等肉眼可直接看出的语法问题
2. 英文拼写错误：变量名/函数名中明确的拼写错误（如 “recieve”→”receive”，”defualt”→”default”）

【绝对禁止】
- 禁止对 “-” 删除行或无前缀上下文行提意见
- 禁止提出”可能”、”建议”、”潜在”、”考虑”、”或许”、”应该”等不确定表述
- 禁止提出代码风格、命名规范、性能、安全、逻辑等任何非语法/拼写类的意见
- 禁止提出需要理解业务逻辑才能判断的意见
- 禁止提出修改前后内容相同的建议（如 error→error）
- 如果不能从当前代码直接看出确定错误，就必须回复 “LGTM”，宁可漏报不可误报
